...
void main()
{
	...
	diff = max(dot(normal, lightdir), 0.0); // Calculates the diffuse lighting

	vec3 viewDir = normalize(-fragPos); // Normalized vector pointing from the fragment position 
	vec3 reflectDir = reflect(-lightdir, normal); // Reflection direction
	spec = pow(max(dot(viewDir, reflectDir), 0.0), phongExp); // Calculates the specular highlight strength 
